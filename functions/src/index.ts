/* eslint-disable */
// index.ts
// Versão COMPLETA FINAL (sem omissões e com scrapers REATIVADOS)
// Inclui ferramentas RapidAPI, Scrapers, Notícias HLTV, Comandos Rápidos
// e Prompt Refinado v5 (temp 0.5).
...
async function executeGetFuriaRoster(): Promise<z.infer<typeof furiaRosterOutputSchema>> {
  ...
  const CACHE_TTL_SUCCESS = 14400; const CACHE_TTL_ERROR = 3600;
  let hltvResult: z.infer<typeof rosterCacheSchema> | null = null; let isCloudflareBlock = false;

  if (redis) { try { const d=await redis.get(hltvCacheKey); if(d){ try { const p=rosterCacheSchema.parse(JSON.parse(d)); if(!p.error){ console.info(`[Cache HLTV] hit ${hltvCacheKey}`); return {playersInfo: p.playersInfo, source:'HLTV'}; } else if(p.error.includes('Cloudflare')||p.error.includes('Access denied')){isCloudflareBlock=true; console.warn(`[Cache HLTV] hit erro CF ${hltvCacheKey}`);} else {console.warn(`[Cache HLTV] hit erro ${hltvCacheKey}`);} } catch (pE){ console.warn(`[Cache HLTV] parse err ${hltvCacheKey}`,pE); } } else {console.info(`[Cache HLTV] miss ${hltvCacheKey}`);} } catch (e){console.error(`[Cache HLTV] read err ${hltvCacheKey}`,e);} }

  if (!isCloudflareBlock) {
    console.info("[HLTV API] Tentando buscar dados...");
    try {
      const team = await HLTV.getTeam({ id: 8297 }); if (!team?.players?.length) throw new Error("Dados HLTV não encontrados.");
      ...
      const playersInfo = players.join(', '); console.info(`[HLTV API] Sucesso: ${playersInfo}`); hltvResult = { playersInfo: playersInfo, source: 'hltv' };
  ...
  try {
    ...
    const $ = cheerio.load(htmlContent); const players: string[] = []; const activeHeader = $('h3 > span#Active'); if (activeHeader.length === 0) throw new Error("Header 'Active' não encontrado."); const rosterTableWrapper = activeHeader.closest('h3').nextAll('div.table-responsive.roster-card-wrapper').first(); const rosterTable = rosterTableWrapper.find('table.wikitable.roster-card').first(); if (rosterTable.length === 0) throw new Error("Tabela roster-card não encontrada.");
    ...
    if (players.length > 0) { const info = players.join(', '); console.info("[Liquipedia API] Sucesso (Roster):", info); liquipediaResult = { playersInfo: info, source: 'liquipedia' }; } else { throw new Error("Extração tabela 'Active' vazia."); }
...
const getFuriaRosterTool = ai.defineTool({ name: "getFuriaRoster", description: "Busca a escalação ATUAL da FURIA CS2 (HLTV/Liquipedia). SEMPRE use esta ferramenta para perguntas sobre o elenco.", inputSchema: z.object({}), outputSchema: furiaRosterOutputSchema }, executeGetFuriaRoster);
...
async function executeSearchWikipedia(input: z.infer<typeof wikipediaSearchSchema>): Promise<z.infer<typeof wikipediaOutputSchema>> {
  ...
  try { wiki.setLang('pt'); const page = await wiki.page(searchTerm, { autoSuggest: true });
    ...
    else { const summaryResult = await page.summary(); if (!summaryResult?.extract) { console.warn(`[Wiki API] Resumo vazio ${searchTerm}.`); apiResult = { error: `Não foi possível obter um resumo para '${searchTerm}'.` }; }
...
const searchWikipediaTool = ai.defineTool({ name: "searchWikipedia", description: "Busca um resumo na Wikipedia (jogador, time, evento). Use SEMPRE para perguntas sobre pessoas específicas.", inputSchema: wikipediaSearchSchema, outputSchema: wikipediaOutputSchema }, executeSearchWikipedia);
...
const getFuriaUpcomingMatchesRapidAPITool = ai.defineTool({ name: "getFuriaUpcomingMatchesRapidAPI", description: "Busca as próximas 3 partidas da FURIA CS2 (Fonte: API Externa). Use esta ferramenta OBRIGATORIAMENTE para próximos jogos.", inputSchema: z.object({}), outputSchema: upcomingMatchesRapidAPIOutputSchema }, executeGetFuriaUpcomingMatchesRapidAPI);
...
const getFuriaRecentResultsRapidAPITool = ai.defineTool({ name: "getFuriaRecentResultsRapidAPI", description: "Busca os 5 resultados mais recentes da FURIA CS2 (Fonte: API Externa). Use esta ferramenta OBRIGATORIAMENTE para resultados.", inputSchema: z.object({}), outputSchema: recentResultsRapidAPIOutputSchema }, executeGetFuriaRecentResultsRapidAPI);

// --- Ferramenta Próximas Partidas (Liquipedia Scraper) ---
const upcomingMatchesLiquipediaOutputSchema = z.object({ matchesInfo: z.string().optional().describe("String com próximas partidas da Liquipedia."), error: z.string().optional() });
async function executeGetFuriaUpcomingMatchesLiquipedia(): Promise<z.infer<typeof upcomingMatchesLiquipediaOutputSchema>> {
  ...
  try {
    ...
    $('div.fo-nttax-infobox table.infobox_matches_content').first().find('tbody tr').each((_idx, row) => {
      const $row = $(row); const tournamentLink = $row.find('td a').first(); const opponentMaybe = '?'; const tournamentName = tournamentLink.attr('title') || tournamentLink.text().trim() || '?'; const dateTimeElement = $row.find('.timer-object'); const dateTime = dateTimeElement.text().trim() || dateTimeElement.data('timestamp');
...
const getFuriaUpcomingMatchesLiquipediaTool = ai.defineTool({ name: "getFuriaUpcomingMatchesLiquipedia", description: "Busca as próximas 3 partidas da FURIA CS2 (Fonte: Liquipedia Scraper - backup, pode falhar).", inputSchema: z.object({}), outputSchema: upcomingMatchesLiquipediaOutputSchema }, executeGetFuriaUpcomingMatchesLiquipedia);
...
const getFuriaRecentResultsLiquipediaTool = ai.defineTool({ name: "getFuriaRecentResultsLiquipedia", description: "Busca os 5 resultados mais recentes da FURIA CS2 (Fonte: Liquipedia Scraper - backup, pode falhar).", inputSchema: z.object({}), outputSchema: recentResultsLiquipediaOutputSchema }, executeGetFuriaRecentResultsLiquipedia);
...
async function executeGetFuriaNewsHltv(): Promise<z.infer<typeof hltvNewsOutputSchema>> {
  ...
  try {
    ...
    for (const item of feed.items) {
      ...
      if (isRelevant && item.title && link) { furiaNews.push(`${item.title.trim()}: ${link}`); if (furiaNews.length >= 5) break; }
    }
  ...
  } catch (error: any) { console.error("[HLTV RSS] Erro ao buscar ou processar feed:", error.message); result = { error: `Falha feed HLTV: ${error.message}` }; }
...
const getFuriaNewsHltvTool = ai.defineTool({ name: "getFuriaNewsHltv", description: "Busca as 5 notícias mais recentes sobre a FURIA no feed RSS da HLTV. Use para perguntas sobre notícias ou novidades.", inputSchema: z.object({}), outputSchema: hltvNewsOutputSchema }, executeGetFuriaNewsHltv);
...
const activeTools = [
  ...
  getFuriaUpcomingMatchesLiquipediaTool, // <-- REATIVADA
  getFuriaRecentResultsLiquipediaTool,    // <-- REATIVADA
  getFuriaNewsHltvTool
];
console.info(`Ferramentas Genkit ATIVAS (${activeTools.length}): ${activeTools.map(t=>t.name).join(', ')}`);
...
const furiaChatFlow = defineFlow(
  ...
  async (input): Promise<string> => {
    ...
    // ***** PROMPT FINAL REFINADO (v4 - Mantido, pois o v5 não funcionou) *****
    const systemInstruction = `Você é FURIOSO, o assistente virtual oficial e super fã da FURIA Esports! Sua missão é ajudar a galera com informações precisas e atualizadas sobre nosso time de CS2, sempre com muito entusiasmo! Lembre-se do nosso papo anterior pra gente continuar na mesma página! 😉
        ...
        - **Uso OBRIGATÓRIO das Ferramentas (Sua Caixa de Habilidades! 🛠️):** Você DEVE usar as ferramentas certas para buscar informações atualizadas.
            - **Escalação ATUAL?** Use OBRIGATORIAMENTE 'getFuriaRoster'. A informação da ferramenta SEMPRE sobrepõe o que você 'lembra'.
            - **Próximos Jogos?** É OBRIGATÓRIO tentar usar 'getFuriaUpcomingMatchesRapidAPI' e 'getFuriaUpcomingMatchesLiquipedia'. Use a informação de ambas se disponíveis.
            - **Resultados Recentes?** É OBRIGATÓRIO tentar usar 'getFuriaRecentResultsRapidAPI' e 'getFuriaRecentResultsLiquipedia'. Use a informação de ambas se disponíveis.
            - **Notícias/Novidades?** É OBRIGATÓRIO usar 'getFuriaNewsHltv'.
            - **Alguém Específico (Jogador/Coach/Staff)?** É OBRIGATÓRIO usar 'searchWikipedia'.
            - **Outros Tópicos (Torneios, Conceitos CS)?** Use 'searchWikipedia'.
        - **Como Responder (O mais importante!):**
            - **SEM METALINGUAGEM!** NUNCA, JAMAIS, em hipótese alguma, mencione que você "usou uma ferramenta", "buscou na API", "pesquisou na Wikipedia", etc. Aja como se você soubesse a informação. Apresente o resultado DIRETAMENTE!
    ...
    try {
      const toolsToUse = activeTools; // Usa a lista de ferramentas ativas
      console.info(`[Flow] Chamando ai.generate com ${messagesForAI.length} mensagens e ${toolsToUse.length} ferramentas.`);

      // ***** TEMPERATURA 0.5 *****
      let llmResponse = await ai.generate({ model: gemini15Flash, messages: messagesForAI, tools: toolsToUse, config: { temperature: 0.5 } });
      let attempts = 0;
      const MAX_TOOL_ATTEMPTS = 3;

      while (attempts < MAX_TOOL_ATTEMPTS) {
        const responseMessage = llmResponse.message;
        // Ajuste: Verificar se responseMessage existe ANTES de acessar content
        if (!responseMessage || !responseMessage.content || !Array.isArray(responseMessage.content)) {
          const directText = llmResponse.text; if (directText) { console.warn("[Flow] Usando llmResponse.text direto (sem tool call ou content inválido)."); return directText; }
          console.error("[Flow] Resposta IA inválida (sem message/content):", llmResponse); return "Oloco! Deu ruim na resposta da IA. 😵";
        }

        const toolRequestParts = responseMessage.content.filter(part => part.toolRequest);
        if (toolRequestParts.length === 0) {
          const finalText = llmResponse.text; console.info(`[Flow] Resposta final IA (s/ ferramenta): "${finalText?.substring(0, 100)}..."`);
          // Adiciona verificação para garantir que não está retornando placeholder de erro interno
          if (!finalText || finalText.includes("CRASHEI!") || finalText.includes("Oloco!")) {
            return "Hmm, parece que me confundi aqui. Pode perguntar de novo?";
          }
          return finalText;
        }
        ...
        messagesForAI.push(responseMessage); // Adiciona a REQUISIÇÃO da IA ao histórico
        const toolResponses: MessageData[] = [];

        // Executar ferramentas solicitadas em paralelo
        const toolPromises = toolRequestParts.map(async (part) => {
          ...
          console.info(`[Flow] Preparando para executar: ${toolName} input:`, JSON.stringify(inputArgs));
          let executor: Function | undefined; let requiresInput = false; let toolDefinition: any = undefined;

          const activeTool = activeTools.find(t => t.name === toolName);
          if (activeTool) {
            if (toolName === getFuriaRosterTool.name) executor = executeGetFuriaRoster;
            else if (toolName === searchWikipediaTool.name) { executor = executeSearchWikipedia; requiresInput = true; }
            else if (toolName === getFuriaUpcomingMatchesRapidAPITool.name) executor = executeGetFuriaUpcomingMatchesRapidAPI;
            else if (toolName === getFuriaRecentResultsRapidAPITool.name) executor = executeGetFuriaRecentResultsRapidAPI;
            else if (toolName === getFuriaUpcomingMatchesLiquipediaTool.name) executor = executeGetFuriaUpcomingMatchesLiquipedia; // Reativada
            else if (toolName === getFuriaRecentResultsLiquipediaTool.name) executor = executeGetFuriaRecentResultsLiquipedia; // Reativada
            else if (toolName === getFuriaNewsHltvTool.name) executor = executeGetFuriaNewsHltv;
          ...
          if (executor && toolDefinition) {
            try {
              ...
              console.info(`[Flow] EXECUTANDO ${toolName}...`);
              output = await executor(parsedInput);
              console.info(`[Flow] Resultado ${toolName}:`, JSON.stringify(output).substring(0, 100) + "..."); // Log mais curto
            } catch (error) {
              if (error instanceof z.ZodError) { console.warn(`[Flow] Input inválido IA ${toolName}:`, inputArgs, error.errors); output = { error: `Input IA inválido: ${error.errors.map((e: ZodIssue) => e.message).join(', ')}` }; }
              else { console.error(`[Flow] Erro ferramenta ${toolName}:`, error); output = { error: `Erro interno ${toolName}: ${error instanceof Error ? error.message : String(error)}` }; }
            }
          } else { console.warn(`[Flow] Ferramenta ${toolName} não encontrada/ativa.`); output = { error: `Ferramenta '${toolName}' desconhecida ou desativada.` }; }
          // Retorna a estrutura completa para Promise.all
        ...
        // Adiciona as respostas ao histórico para a próxima chamada da IA
        messagesForAI.push(...resolvedResponses.filter((r): r is MessageData => r !== null));

        console.info(`[Flow] Rechamando ai.generate com ${resolvedResponses.length} resposta(s) de ferramenta(s).`);
...
function formatToolResponseForUser(toolName: string, response: any): string {
  if (!response) return `Deu ruim aqui tentando buscar ${toolName}. Sem resposta da ferramenta. 😥`; if (response.error) { return `Ops! Tive um problema ao buscar ${toolName}: ${response.error} 😥`; }
  if (toolName === 'getFuriaRoster' && response.playersInfo) { return `Nosso elenco atual é: ${response.playersInfo} (Fonte: ${response.source || '?'})! 🔥`; }
  if (toolName === 'getFuriaUpcomingMatchesRapidAPI' && response.matchesInfo) { if (response.matchesInfo.startsWith("Nenhuma")) return `Pela API, ${response.matchesInfo} Fica ligado nas redes! 👀`; return `Próximos jogos (API):\n- ${response.matchesInfo.replace(/;\s*/g, '\n- ')}`; }
  if (toolName === 'getFuriaRecentResultsRapidAPI' && response.resultsInfo) { if (response.resultsInfo.startsWith("Nenhuma")) return `Pela API, ${response.resultsInfo} Confere no HLTV que deve ter algo lá! 🤔`; return `Resultados recentes (API):\n- ${response.resultsInfo.replace(/;\s*/g, '\n- ')}`; }
  if (toolName === 'getFuriaNewsHltv' && response.newsInfo) { if (response.newsInfo.startsWith("Nenhuma")) return `Não achei notícias fresquinhas da FURIA no feed da HLTV agora. 📰`; return `Últimas notícias da HLTV:\n- ${response.newsInfo.replace(/;\s*/g, '\n- ')} 📰`; }
  return `Resultado de ${toolName}: ${JSON.stringify(response)}`;
}

// --- Configuração do Servidor Express e Webhook com Comandos ---
const app = express(); app.use(express.json()); app.get('/', (_req, res) => { res.status(200).send('Servidor Bot Furia CS Ativo!'); });
const WEBHOOK_PATH = `/telegram/webhook/${telegramToken}`; console.info(`Configurando POST para webhook em: ${WEBHOOK_PATH}`);
app.post(WEBHOOK_PATH, async (req, res) => {
  const update: TelegramBot.Update = req.body; if (!update || !update.message || !update.message.chat?.id) { console.info(`[Webhook] Update inválido.`); res.sendStatus(200); return; }
  const chatId = update.message.chat.id; if (update.message.from?.is_bot) { console.info(`[Webhook] Update de bot ignorado.`); res.sendStatus(200); return; }
  res.sendStatus(200);

  // LÓGICA DE COMANDOS RÁPIDOS
  if (update.message.text && update.message.text.startsWith('/')) {
    const command = update.message.text.split(' ')[0].toLowerCase(); console.info(`[Webhook] Comando ${command} chat ${chatId}`);
    let toolResponse: any; let toolName = ''; let processingMessage = ''; let executorFunction: (() => Promise<any>) | undefined;
    switch (command) {
    case '/elenco': case '/roster': toolName = 'getFuriaRoster'; processingMessage = 'Buscando o elenco... 🐾'; executorFunction = executeGetFuriaRoster; break;
    case '/proximojogo': case '/proximapartida': case '/next': toolName = 'getFuriaUpcomingMatchesRapidAPI'; processingMessage = 'Conferindo a agenda (API)... 🔥'; executorFunction = executeGetFuriaUpcomingMatchesRapidAPI; break;
    case '/ultimojogo': case '/ultimoresultado': case '/last': toolName = 'getFuriaRecentResultsRapidAPI'; processingMessage = 'Consultando os resultados (API)... 🏆'; executorFunction = executeGetFuriaRecentResultsRapidAPI; break;
    case '/noticias': case '/news': toolName = 'getFuriaNewsHltv'; processingMessage = 'Buscando as últimas notícias da HLTV... 📰'; executorFunction = executeGetFuriaNewsHltv; break;
    default: await bot.sendMessage(chatId, `Comando "${command}" não reconhecido. 🤔 Tente /elenco, /proximojogo, /ultimoresultado ou /noticias.`); return;
    }
    if (executorFunction) { try { await bot.sendMessage(chatId, processingMessage); toolResponse = await executorFunction(); const formattedReply = formatToolResponseForUser(toolName, toolResponse); await bot.sendMessage(chatId, formattedReply); console.info(`[Webhook] Resposta comando ${command} enviada chat ${chatId}.`); } catch (error) { console.error(`[Webhook] Erro comando ${command} chat ${chatId}:`, error); await bot.sendMessage(chatId, `Putz, deu erro no comando ${command}. 🤯`); } } return;
  }
  // Trata mensagens de texto normais (IA)
  else if (update.message.text) {
    const userMessage = update.message.text.trim(); console.info(`[Webhook] Msg (IA) chat ${chatId}: "${userMessage}"`); const contextKey = `genkit_history:${chatId}`; let historyForFlow: MessageData[] = [];
    if (redis) { try { const d=await redis.get(contextKey); if(d){try{const p=JSON.parse(d);if(Array.isArray(p)){historyForFlow=p.filter(m=>m&&typeof m.role==='string'&&Array.isArray(m.content));console.info(`[Webhook] Histórico Redis ${chatId} (${historyForFlow.length})`);}else{await redis.del(contextKey);}}catch(pE){await redis.del(contextKey);}}}catch(rE){console.error(`Erro Redis read ${chatId}:`, rE);}}
    try {
      await bot.sendChatAction(chatId, "typing"); const flowResult = await runFlow(furiaChatFlow, { userMessage: userMessage, chatHistory: historyForFlow });
      const finalReply = flowResult; console.info(`[Webhook] Flow raw: "${finalReply.substring(0, 200)}..."`);
      const lastUser:MessageData = { role: 'user', content: [{ text: userMessage }] }; const lastModel:MessageData = { role: 'model', content: [{ text: finalReply }] }; const histToSave = [...historyForFlow, lastUser, lastModel]; const MAX_HIST=8; while(histToSave.length>MAX_HIST){histToSave.shift();}
      if(redis){try{await redis.set(contextKey,JSON.stringify(histToSave),'EX',1800);console.info(`[Webhook] Histórico (${histToSave.length}) salvo Redis ${chatId}`);}catch(rE){console.error(`Erro Redis save ${chatId}:`, rE);}}
      try { await bot.sendMessage(chatId, finalReply); console.info(`[Webhook] Resposta IA enviada chat ${chatId}.`); } catch (tE) { console.error(`Erro Telegram send ${chatId}:`, tE); }
    } catch (flowError) { console.error(`[Webhook] Erro GERAL flow ${chatId}:`, flowError); try { await bot.sendMessage(chatId, "⚠️ Putz! Deu ruim aqui na máquina! 🤖💥 Tenta mandar a pergunta de novo?"); } catch (sE) { console.error("Falha CRÍTICA erro final", chatId, sE); } }
  }
  // Trata stickers
  else if (update.message.sticker) { console.info(`[Webhook] Sticker chat ${chatId}.`); try { await bot.sendMessage(chatId, "Que sticker maneiro! 🤩 Mas ó, eu funciono melhor com mensagens de texto pra te ajudar com infos da FURIA, beleza? 😉"); } catch (e) { console.error(`Erro sticker resp ${chatId}:`, e); } }
  // Trata outros tipos
  else { const type = Object.keys(update.message).filter(k=>!['message_id','from','chat','date'].includes(k))[0]||'?'; console.info(`[Webhook] Tipo ${type} não suportado ${chatId}.`); try { await bot.sendMessage(chatId, "Hmm, esse tipo de mensagem eu não manjo muito. 😅 Manda em texto, por favor? 👍"); } catch (e) { console.error(`Erro resp tipo ${type} ${chatId}:`, e); } }
});
...
const server = app.listen(numericPort, host, () => { console.info(`Servidor Express escutando em https://${host}:${numericPort}`); console.info(`Webhook Telegram: ${WEBHOOK_PATH}`); });
// --- Encerramento Gracioso ---
const gracefulShutdown = (signal: string) => { console.info(`${signal} signal: closing server...`); server.close(async () => { console.info('HTTP server closed.'); if (redis) { try { await redis.quit(); console.info('Redis closed.'); } catch (e) { console.error('Erro Redis quit:', e); process.exitCode = 1; } } console.info('Exiting.'); process.exit(); }); setTimeout(() => { console.error("Timeout shutdown"); process.exit(1); }, 10000); };
